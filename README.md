...
| `--skip-existing`           | Skip conversion of series whose NIfTI outputs already exist with matching provenance (source data, software versions, and lookup table). Series are reconverted if any of these have changed. Note: adding new data to the source may change series numbering, which can leave orphan files from previous naming. Orphan files are logged as warnings. Use `--force` for a clean reconversion. | `False`                                           |
